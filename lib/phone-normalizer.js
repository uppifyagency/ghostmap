 ...
 * normalizePhone('0612345678')    // '+33612345678' (French mobile, no context)
...
export function normalizePhone(phone, options = {}) {
    ...
    // When country context is provided, use it to disambiguate.
    if (/^0[67]\d{8}$/.test(cleaned)) {
        ...
        // No context: preserve backward-compatible behavior (French mobile without spaces)
 ...
 */
export function formatPhoneForCsv(phone) {
    if (!phone) return '';
